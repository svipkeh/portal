...
(function($) {
	...
	$.fn.paginator = function(options) {
		...
		return this.each(function() {
			...
			// empty out the destination element and then render out the pager with
			// the supplied options
			$(this).empty().append(
					renderpager(parseInt(options.current_page),
							parseInt(options.page_count), parseInt(options.button_number),
							parseInt(options.total_count),parseInt(options.page_size), options.pager_click));

			// append quick jump
			// $(this).append(quickjump(parseInt(options.current_page),
			// parseInt(options.page_count), parseInt(options.button_number),
			// options.pager_click));
	...
	function renderpager(current_page, page_count, button_number, total_count, page_size, pager_click) {

		var $pager = $('<ul class=\'pagination\'></ul>');
		// add info
		$pager.append('<li class=\'disabled\'><a>共' + total_count + '条数据，当前'
				+ current_page + '/' + page_count + '页，每页' + page_size +'条</a><li>');
		// add first & prev
		$pager.append(renderButton('first', current_page, page_count, pager_click))
		...
		// add last & next
		$pager.append(renderButton('next', current_page, page_count, pager_click))
	...
	function quickjump(current_page, page_count, button_number, pager_click) {
		...
		for (var i = 1; i <= page_count; i++) {
			if (i == current_page)
				...
				var attr = "";
			select
					.append('<option ' + attr + ' value="' + i + '">' + i + '</option>');
		}
		select.change(function() {
			pager_click(this.value);
		});
		return jump.append(
				'<span style="margin-left:15px;margin-bottom:5px;">跳转至 </span').append(
				select);
	}
	...
	function renderButton(link_page, current_page, page_count, pager_click) {
		...
		return btn = $(
				'<li class="' + class_li + '"><a data-toggle="tooltip" ' + title
						+ ' href="#">' + link_content + '</a></li>').click(callback);
	}
